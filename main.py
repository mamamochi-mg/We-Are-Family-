...
async def receive_url(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if thumbnail:
            try:
                photo_message = await context.bot.send_photo(status.chat_id, photo=thumbnail, caption=preview, parse_mode="HTML", reply_markup=format_keyboard(token))
                await message.delete()
            except Exception:
                log.exception("thumbnail preview failed; falling back to text preview")
                await context.bot.edit_message_text(preview, chat_id=status.chat_id, message_id=status.message_id, parse_mode="HTML", reply_markup=format_keyboard(token))
        else:
